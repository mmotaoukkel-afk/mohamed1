...
export const ensureUserDocument = async (userData) => {
    ...
    try {
        ...
        if (!userDoc.exists()) {
            // 🛡️ Pre-migration Check: Look for any "pending" admin document by email
            let role = userData.email === 'admin@kataraa.com' ? USER_ROLES.SUPER_ADMIN : USER_ROLES.CUSTOMER;
            ...
            try {
                ...
                if (!snapshot.empty) {
                    // Found a pre-authorized admin document!
                    const pendingDoc = snapshot.docs.find(d => d.id !== userData.uid);
                    if (pendingDoc) {
                        const pendingData = pendingDoc.data();
                        console.log(`🚀 Found pending admin authorization for ${userData.email}. Migrating...`);
                        role = pendingData.role || role;
                        displayName = pendingData.displayName || displayName;

                        // Delete the old "temporary" admin document
                        await deleteDoc(doc(db, 'users', pendingDoc.id));
                    }
                }
            } catch (e) {
                console.error('Migration search failed:', e);
            }

            // Create new user document with correct role
            await setDoc(userRef, {
...
export const getAllAdmins = async () => {
    try {
        ...
        const admins = snapshot.docs.map(doc => ({
            ...
            id: doc.id, // ID after prevents overwriting
